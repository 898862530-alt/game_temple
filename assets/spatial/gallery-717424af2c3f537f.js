...
export function mount(host){
 ...
 host.prepend(renderer.domElement);const canvas=renderer.domElement;canvas.tabIndex=0;canvas.setAttribute('aria-label','死亡搁浅三维展厅，拖动或使用长按方向键平滑移动，使用加减号缩放');
 ...
 let transition=null,disposed=false,frame=0,last=0,view='lobby',room='lobby',rainEnabled=!reduced;
 const keys=new Set(),velocity=new T.Vector3(),direction=new T.Vector3(),forward=new T.Vector3(),right=new T.Vector3();
 const events=new AbortController();const on=(el,type,fn)=>el.addEventListener(type,fn,{signal:events.signal});
 const motion=host.querySelector('#spatial-motion'),rb=host.querySelector('#spatial-rain');
 ...
 function select(name){
  ...
  const [p,t]=presets[name];transition={p:new T.Vector3(...p),t:new T.Vector3(...t)};
  ...
  rb.disabled=room==='lobby';
  if(reduced){camera.position.copy(transition.p);controls.target.copy(transition.t);transition=null;controls.update();}request();
 }
 ...
 on(motion,'click',()=>{stop();transition=null;controls.autoRotate=!controls.autoRotate;motion.setAttribute('aria-pressed',String(controls.autoRotate));request()});
 ...
 on(canvas,'pointerdown',e=>{stop();transition=null;pauseOrbit();pointerStart=[e.clientX,e.clientY];canvas.focus({preventScroll:true});});
 on(canvas,'pointerup',e=>{if(room!=='lobby'||!pointerStart||Math.hypot(e.clientX-pointerStart[0],e.clientY-pointerStart[1])>6)return;const r=canvas.getBoundingClientRect();raycaster.setFromCamera(new T.Vector2((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1),camera);if(raycaster.intersectObjects([portal,icon],true).length)select('hall');pointerStart=null;});
 on(canvas,'wheel',()=>{transition=null;stop();pauseOrbit();});
 const accepted=['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-','='];
 on(canvas,'keydown',e=>{if(!accepted.includes(e.key))return;e.preventDefault();transition=null;pauseOrbit();keys.add(e.key);request();});
 on(window,'keyup',e=>keys.delete(e.key));on(canvas,'blur',stop);on(window,'blur',stop);
 ...
 function move(dt){
  ...
  direction.set(0,0,0).addScaledVector(forward,Number(keys.has('ArrowUp'))-Number(keys.has('ArrowDown'))).addScaledVector(right,Number(keys.has('ArrowRight'))-Number(keys.has('ArrowLeft')));
  if(direction.lengthSq()>0)direction.normalize().multiplyScalar(6);
  velocity.lerp(direction,1-Math.exp(-(keys.size?5:7)*dt));if(velocity.lengthSq()<.00001)velocity.set(0,0,0);
  const delta=velocity.clone().multiplyScalar(dt),next=controls.target.clone().add(delta);next.x=T.MathUtils.clamp(next.x,-18,18);next.z=T.MathUtils.clamp(next.z,-16,18);delta.copy(next).sub(controls.target);controls.target.add(delta);camera.position.add(delta);
  const zoom=Number(keys.has('-'))-Number(keys.has('+')||keys.has('='));if(zoom){const offset=camera.position.clone().sub(controls.target);offset.setLength(T.MathUtils.clamp(offset.length()*Math.exp(zoom*dt),3,48));camera.position.copy(controls.target).add(offset);}
  return keys.size>0||velocity.lengthSq()>0;
 }
 ...
 on(captureButton,'click',async()=>{
  ...
  try{
   ...
   ctx.strokeStyle='#b9c0b7';ctx.beginPath();ctx.moveTo(pad,y+w*.14);ctx.lineTo(w-pad,y+w*.14);ctx.stroke();ctx.font=`${w*.01}px monospace`;ctx.fillText(`${new Date().toISOString().slice(0,10)}  /  ${capturedRoom==='lobby'?'ATRIUM':capturedRoom==='hall'?'EXHIBITION':'WILDERNESS'}  /  v1.1.0`,pad,y+w*.163);ctx.textAlign='right';ctx.fillText('KEEP ON KEEPING ON',w-pad,y+w*.163);
 ...
 function tick(now){frame=0;if(disposed||document.hidden)return;const dt=Math.min((now-last)/1000,.05);last=now;if(transition){camera.position.lerp(transition.p,1-Math.exp(-5*dt));controls.target.lerp(transition.t,1-Math.exp(-5*dt));if(camera.position.distanceTo(transition.p)<.015){camera.position.copy(transition.p);controls.target.copy(transition.t);transition=null;}}const moving=move(dt);const changed=controls.update();if(rain.visible&&!reduced){for(let i=0;i<900;i++){let j=i*6;rainP[j+1]-=dt*3;rainP[j+4]-=dt*3;if(rainP[j+1]<1.5){rainP[j+1]+=10;rainP[j+4]+=10;}}rainGeo.attributes.position.needsUpdate=true;}renderer.render(scene,camera);if(moving||transition||controls.autoRotate||(rain.visible&&!reduced)||changed)request();}
