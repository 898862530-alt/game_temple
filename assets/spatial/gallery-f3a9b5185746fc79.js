...
export function mount(host){
 ...
 try{renderer=new T.WebGLRenderer({antialias:devicePixelRatio<2,powerPreference:'high-performance'});}catch(e){status.textContent='当前设备无法开启三维画面。可在下方阅读展览说明与作品评测。';return ()=>{};}
 ...
 const controls=new OrbitControls(camera,canvas);controls.target.set(0,3,-3);controls.enableDamping=true;controls.enablePan=false;controls.minDistance=3;controls.maxDistance=48;controls.maxPolarAngle=Math.PI-.12;controls.autoRotateSpeed=.45;
 ...
 const presets={lobby:[[12,8,22],[0,3,-3]],hall:[[20,7,26],[0,2.4,0]],land:[[10,5,10],[0,2.5,-3]],bridge:[[5,4,6],[1,1.9,river(1)]],porter:[[5.7,3.7,6],[3,2.7,3]]};
 ...
 function clampCinema(){if(mode!=='cinema')return;const floor=room==='land'&&Math.abs(camera.position.x)<=12&&Math.abs(camera.position.z)<=10?height(camera.position.x,camera.position.z):-.15;if(camera.position.y<floor){camera.position.y=floor;camera.lookAt(controls.target);}}
 ...
 function select(name){
  if(!presets[name])return;stop();pauseOrbit();const damping=controls.enableDamping;controls.enableDamping=false;controls.update();controls.enableDamping=damping;view=name;
  if(name==='lobby')room='lobby';else if(name==='hall')room='hall';else if(name==='land')room='land';else if(room==='lobby')room='hall';
  renderer.shadowMap.needsUpdate=true;lobby.visible=room==='lobby';world.visible=room!=='lobby';hall.visible=room!=='land';pedestal.visible=room==='hall';rain.visible=rainEnabled&&room!=='lobby';
 ...
 const dofButton=host.querySelector('#spatial-dof'),apertureInput=host.querySelector('#spatial-aperture'),focusInput=host.querySelector('#spatial-focus'),pickButton=host.querySelector('#spatial-focus-pick');
 let dofEnabled=false,pickFocus=false,post=null;
 function makePost(){
  if(post)return;
  ...
  const uniforms={colorMap:{value:target.texture},depthMap:{value:target.depthTexture},resolution:{value:new T.Vector2(1,1)},nearClip:{value:camera.near},farClip:{value:camera.far},focusDistance:{value:18},fStop:{value:5.6},focalLength:{value:35}};
  const material=new T.ShaderMaterial({uniforms,depthTest:false,depthWrite:false,vertexShader:`varying vec2 vUv;void main(){vUv=uv;gl_Position=vec4(position.xy,0.,1.);}`,fragmentShader:`
   varying vec2 vUv;uniform sampler2D colorMap,depthMap;uniform vec2 resolution;uniform float nearClip,farClip,focusDistance,fStop,focalLength;
   float depthAt(vec2 uv){float d=texture2D(depthMap,uv).x;return nearClip*farClip/(farClip-d*(farClip-nearClip));}
   float coc(float z){float f=focalLength*.001;return min(10.,abs(f*f*(z-focusDistance)/(fStop*z*max(focusDistance-f,.01)))*resolution.x/.036*.5);}
   void main(){float z=depthAt(vUv),radius=coc(z);vec3 color=texture2D(colorMap,vUv).rgb;float weight=1.;
    for(int i=0;i<16;i++){float a=float(i)*2.39996323;vec2 delta=vec2(cos(a),sin(a))*sqrt((float(i)+.5)/16.)*radius;vec2 uv=clamp(vUv+delta/resolution,vec2(0.),vec2(1.));float dz=depthAt(uv);float w=dz<z?clamp(coc(dz)/(length(delta)+.001),0.,1.):1.;color+=texture2D(colorMap,uv).rgb*w;weight+=w;}
    gl_FragColor=vec4(color/weight,1.);
    #include <tonemapping_fragment>
    #include <colorspace_fragment>
   }`});
  const screen=new T.Scene(),quad=new T.Mesh(new T.PlaneGeometry(2,2),material);screen.add(quad);post={target,uniforms,material,screen,quad,camera:new T.Camera()};sizePost();
 }
 function sizePost(){if(!post)return;const size=renderer.getDrawingBufferSize(new T.Vector2());post.target.setSize(size.x,size.y);post.uniforms.resolution.value.copy(size);}
 ...
 function renderFrame(){if(!dofEnabled){renderer.render(scene,camera);return;}makePost();post.uniforms.fStop.value=Number(apertureInput.value);post.uniforms.focusDistance.value=Number(focusInput.value);post.uniforms.focalLength.value=18/(Math.tan(T.MathUtils.degToRad(camera.fov/2))*camera.aspect);renderer.setRenderTarget(post.target);renderer.render(scene,camera);renderer.setRenderTarget(null);renderer.render(post.screen,post.camera);}
 on(dofButton,'click',()=>{dofEnabled=!dofEnabled;dofButton.setAttribute('aria-pressed',String(dofEnabled));dofButton.textContent='景深 '+(dofEnabled?'开':'关');if(!dofEnabled){pickFocus=false;pickButton.setAttribute('aria-pressed','false');pickButton.textContent='点击对焦';post?.target.dispose();post?.quad.geometry.dispose();post?.material.dispose();post=null;}request();});
 on(apertureInput,'input',()=>{host.querySelector('#spatial-aperture-value').textContent='F'+Number(apertureInput.value).toFixed(1);request();});
 ...
 on(captureButton,'click',async()=>{
  ...
  try{
   ...
   ctx.strokeStyle='#b9c0b7';ctx.beginPath();ctx.moveTo(pad,y+w*.14);ctx.lineTo(w-pad,y+w*.14);ctx.stroke();ctx.font=`${w*.01}px monospace`;ctx.fillText(`${new Date().toISOString().slice(0,10)}  /  ${capturedRoom==='lobby'?'ATRIUM':capturedRoom==='hall'?'EXHIBITION':'WILDERNESS'}  /  v1.3.0`,pad,y+w*.163);ctx.textAlign='right';ctx.fillText('KEEP ON KEEPING ON',w-pad,y+w*.163);
