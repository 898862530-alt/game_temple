...
export function mount(host){
 ...
 on(captureButton,'click',async()=>{
  ...
  try{
   ...
   ctx.strokeStyle='#b9c0b7';ctx.beginPath();ctx.moveTo(pad,y+w*.14);ctx.lineTo(w-pad,y+w*.14);ctx.stroke();ctx.font=`${w*.01}px monospace`;ctx.fillText(`${new Date().toISOString().slice(0,10)}  /  ${capturedRoom==='lobby'?'ATRIUM':capturedRoom==='hall'?'EXHIBITION':'WILDERNESS'}  /  v1.1.1`,pad,y+w*.163);ctx.textAlign='right';ctx.fillText('KEEP ON KEEPING ON',w-pad,y+w*.163);
