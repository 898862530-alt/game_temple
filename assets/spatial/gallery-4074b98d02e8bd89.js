...
export function mount(host){
 ...
 for(const [x,z] of [[-9,5],[8,7],[-6,-9]]){const light=new T.SpotLight('#fff0d6',650,45,.62,.7,1.6);light.position.set(x,13,z);light.target.position.set(x*.3,1.5,z*.25);galleryLights.add(light,light.target);}
 ...
 const distant=new T.Mesh(new T.CylinderGeometry(90,90,85,96,1,true),new T.MeshBasicMaterial({map:distantTexture,side:T.BackSide,fog:false}));distant.position.y=25;backdrop.add(distant);
 const horizonGround=new T.Mesh(new T.PlaneGeometry(200,200),mat('#586451'));horizonGround.rotation.x=-Math.PI/2;horizonGround.position.y=1.05;backdrop.add(horizonGround);
 function lightRoom(next){const outdoor=next==='land';galleryLights.visible=!outdoor;backdrop.visible=outdoor;ambient.intensity=outdoor?.85:.32;ambient.color.set(outdoor?'#cadde9':'#eee8de');sun.intensity=outdoor?2.4:.55;sun.color.set(outdoor?'#dce8e8':'#fff0dd');fill.intensity=outdoor?.45:.18;rim.intensity=outdoor?.8:.25;renderer.toneMappingExposure=outdoor?.95:1.05;scene.background=new T.Color(outdoor?'#aebfc2':'#c3c6c0');scene.fog=new T.Fog(outdoor?'#aebfc2':'#c3c6c0',outdoor?28:55,outdoor?95:130);}
 ...
 const presets={lobby:[[12,8,22],[0,3,-3]],hall:[[16,9,20],[0,2.1,0]],land:[[10,5.2,9],[0,2.2,-2]],bridge:[[5,4,6],[1,1.9,river(1)]],porter:[[5.7,3.7,6],[3,2.7,3]]};
 ...
 function select(name){
  if(!presets[name]||(room==='lobby'&&!['lobby','hall'].includes(name)))return;stop();pauseOrbit();const damping=controls.enableDamping;controls.enableDamping=false;controls.update();controls.enableDamping=damping;view=name;
  if(name==='lobby')room='lobby';else if(name==='hall')room='hall';else if(name==='land')room='land';else if(room==='lobby')room='hall';
  renderer.shadowMap.needsUpdate=true;lobby.visible=room==='lobby';world.visible=room!=='lobby';hall.visible=room!=='land';pedestal.visible=room==='hall';rain.visible=rainEnabled&&room==='land';
  lightRoom(room);
  const [p,t]=(mode==='fps'?fpsPresets:presets)[name];focalInput.value='35';camera.setFocalLength(35);host.querySelector('#spatial-focal-value').textContent='35 mm';updateFocus(new T.Vector3(...p).distanceTo(new T.Vector3(...t)));if(mode==='fps')verticalOffset=0;transition={p:new T.Vector3(p[0],mode==='fps'?groundAt(p[0],p[2])+1.5:p[1],p[2]),t:new T.Vector3(...t),fps:mode==='fps'};
  host.querySelectorAll('[data-view]').forEach(b=>{b.setAttribute('aria-pressed',String(b.dataset.view===name));b.hidden=(room==='lobby'&&!['lobby','hall'].includes(b.dataset.view))||(room==='hall'&&['bridge','porter'].includes(b.dataset.view))});
  ...
  rb.disabled=room!=='land';if(reduced){camera.position.copy(transition.p);controls.target.copy(transition.t);const fps=transition.fps;transition=null;if(fps)syncFirstPerson();else controls.update();}request();
 }
 ...
 let dofEnabled=false,pickFocus=false,post=null,aaEnabled=true,ssrEnabled=!matchMedia('(max-width: 760px)').matches;
 ...
 function makePost(){
  if(post)return;
  const target=new T.WebGLRenderTarget(1,1,{type:T.HalfFloatType,depthBuffer:true,samples:aaEnabled?4:0});target.depthTexture=new T.DepthTexture(1,1,T.UnsignedIntType);
 ...
 on(captureButton,'click',async()=>{
  ...
  try{
   ...
   ctx.strokeStyle='#b9c0b7';ctx.beginPath();ctx.moveTo(pad,y+w*.14);ctx.lineTo(w-pad,y+w*.14);ctx.stroke();ctx.font=`${w*.01}px monospace`;ctx.fillText(`${new Date().toISOString().slice(0,10)}  /  ${capturedRoom==='lobby'?'ATRIUM':capturedRoom==='hall'?'EXHIBITION':'WILDERNESS'}  /  v1.4.0`,pad,y+w*.163);ctx.textAlign='right';ctx.fillText('KEEP ON KEEPING ON',w-pad,y+w*.163);
 ...
 function resize(){const w=host.clientWidth,h=host.clientHeight;renderer.setSize(w,h);camera.aspect=w/h;camera.setFocalLength(Number(focalInput.value)||35);sizePost();request();}const observer=new ResizeObserver(resize);observer.observe(host);
